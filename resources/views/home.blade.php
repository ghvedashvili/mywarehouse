...
@php
    ...
    if ($bestDay) {
        ...
        if ($tdNet >= $bestDay['bdNet']) {
            $rdPhrase      = '🍾 ეგ არის! რეკორდია! დღეს სუფრას შენ გაშლი თუ ჩვენ დაგპატიჟოთ?! 🥂🎉';
            $rdPhraseColor = '#16a34a';
        } elseif ($gap <= 5) {
            $rdPhrase      = "სულ რაღაც {$gap} ორდერი საოცარ ფორმაში ხარ დღეს!";
            ...
            $rdPhrase      = "🚨 {$gap} ნაბიჯი! ასეთი ტემპით თუ გააგრძელე, მალე დირექტორი გახდები! 😎";
            $rdPhraseColor = '#7c3aed';
        } elseif ($gap <= 40) {
            $rdPhrase      = "☕ {$gap} ორდერი გვინდა... ჩართე საყვარელი პლეილისტი და წავედით! 🎶";
            $rdPhraseColor = '#ea580c';
        } elseif ($gap <= 50) {
            $rdPhrase      = "🎯 კიდევ {$gap} ორდერი! დიდი მისიაა, ენერგეტიკული , ორმაგი ყავა ან არ ვიცი რამე ქენი და მიაწექი! 💥";
